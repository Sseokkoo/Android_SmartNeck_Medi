...
public class Fit_BrowserActivity extends AppCompatActivity {
    ...
    public void setUrl(String Token, String Url) {
        Log.d (Fit_Constants.TAG,"----- setUrl: " + Token + " / " + Url);

        if (Token == null) {
            tv_location = (TextView) findViewById(R.id.tv_location);
            tv_location.setText(Fit_Constants.CFG_DOMAIN.replace("http://","") + Url);

            mWebView.loadUrl(Fit_Constants.CFG_DOMAIN + Url);
        } else {
            tv_location = (TextView) findViewById(R.id.tv_location);
            tv_location.setText(Fit_Constants.CFG_DOMAIN.replace("http://","") + Url);

            mWebView.loadUrl(Fit_Constants.CFG_DOMAIN + "/redirect.aspx?MemberToken=" + Token + "&ReturnUrl=" + Url);
        }
    }
